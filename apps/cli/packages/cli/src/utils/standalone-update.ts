...
export function ensurePathInShellRc(binDir: string): ShellPathUpdate {
  ...
  try {
    ...
    const beginMarker = '# LAL PATH block begin';
    const endMarker = '# LAL PATH block end';
    const legacyMarker = '# Added by LAL standalone installer';
...
export async function performStandaloneUpdate(
  ...
  if (fs.existsSync(manifestPath)) {
    ...
    throw new Error(
      `${standaloneDir} exists but is not a LAL standalone install. Remove it manually to proceed.`,
    );
  } else {
