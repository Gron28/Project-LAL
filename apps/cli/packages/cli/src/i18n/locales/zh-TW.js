...
// Traditional Chinese (zh-TW) translations for LAL CLI
...
export default {
  ...
  'User level': '使用者層級',
  'Project level': '專案層級',
  'Clipboard image paste is unavailable because the native clipboard module could not be loaded. Reinstall LAL or use the npm installation method.':
    '剪貼簿圖片貼上功能無法使用，因為原生剪貼簿模組載入失敗。請重新安裝 LAL，或改用 npm 安裝方式。',
  ...
  Bundled: '內建',
  'Available LAL CLI tools:': '可用的 LAL CLI 工具：',
  ...
  'Generate with LAL (Recommended)': '使用 LAL 生成（推薦）',
  ...
  'You are installing an extension from {{originSource}}. Some features may not work perfectly with LAL.':
    '您正在安裝來自 {{originSource}} 的擴展。某些功能可能無法完美兼容 LAL。',
  ...
  'IDE integration is not supported in your current environment. To use this feature, run LAL in one of these supported IDEs: VS Code or VS Code forks.':
    '您當前環境不支持 IDE 集成。要使用此功能，請在以下支持的 IDE 之一中運行 LAL：VS Code 或 VS Code 分支版本。',
  ...
  'This menu is read-only. To add or modify hooks, edit settings.json directly or ask LAL.':
    '此選單為只讀。要添加或修改 Hook，請直接編輯 settings.json 或詢問 LAL。',
  ...
  'To re-enable hooks, remove "disableAllHooks" from settings.json or ask LAL.':
    '要重新啟用 Hook，請從 settings.json 中刪除 "disableAllHooks" 或詢問 LAL。',
  Project: '項目',
  User: '用戶',
  Skill: '技能',
  System: '系統',
  ...
  'Right before LAL concludes its response': 'LAL 結束響應之前',
  ...
  'Voice dictation is not supported on {{platform}}.':
    '語音聽寫不支援 {{platform}}。',
  'Voice dictation needs microphone access, which is unavailable in this WSL session. Use WSLg/PulseAudio, or run LAL on a host with a microphone.':
    '語音聽寫需要麥克風存取，但目前 WSL 會話不可用。請使用 WSLg/PulseAudio，或在具備麥克風的主機上執行 LAL。',
  ...
  'You can run any shell commands from LAL using ! (e.g. !ls).':
    '在 LAL 中使用 ! 可運行任意 shell 命令（例如 !ls）。',
  ...
  'Add a QWEN.md file to give LAL persistent project context.':
    '添加 QWEN.md 檔案，為 LAL 提供持久的項目上下文。',
  ...
  'Agent powering down. Goodbye!': 'LAL 正在關閉，再見！',
  ...
  "Set up LAL's status line UI": '配置 LAL 的狀態欄',
  ...
  'LAL update available! {{current}} → {{latest}}':
    'LAL 有可用更新！{{current}} → {{latest}}',
  'A new version of LAL is available! {{current}} → {{latest}}':
    'LAL 有新版本可用！{{current}} → {{latest}}',
  'LAL {{version}} is up to date!': 'LAL {{version}} 已是最新！',
  ...
  'Update successful! Please restart LAL to use the new version. Switching model providers before restarting may not work correctly.':
    '更新成功！請重新啟動 LAL 以使用新版本。重新啟動前切換模型提供商可能無法正常運作。',
