...
// ---------------------------------------------------------------------------
// Managed-connection detection
// ---------------------------------------------------------------------------

/**
 * True when this install is a managed LAL connection, so the dialog must
 * never fall through to the full Alibaba ModelStudio / third-party onboarding
 * flow (with its ToS link and provider credential steps) — a managed user
 * has no provider credentials of their own to enter, and repairing the
 * connection is a `lal update`, not a picker.
 *
 * `LAL_MANAGED=1` is set by the Windows wrapper (`lal.cmd`) but the
 * Linux/macOS wrapper (`lal.sh`) execs the vendored runtime directly and
 * does not currently export it, which would otherwise leave this dialog's
 * safety net platform-dependent. Falling back to a fingerprint of the
 * downloaded settings — every LAL gateway row carries
 * `envKey: "LAL_API_KEY"` (see `web/src/app/api/lal/client-settings`) — makes
 * the guard self-contained in the fork and correct regardless of what the
 * launcher script does or doesn't export.
 */
function isLalManagedConnection(
  settings: ReturnType<typeof useSettings>,
): boolean {
  if (process.env['LAL_MANAGED'] === '1') return true;
  const providers = settings?.merged?.modelProviders as
    | Record<string, Array<{ envKey?: string }> | undefined>
    | undefined;
  const openaiModels = providers?.['openai'];
  return (
    Array.isArray(openaiModels) &&
    openaiModels.some((entry) => entry?.envKey === 'LAL_API_KEY')
  );
}
...
export function AuthDialog(): React.JSX.Element {
  ...
  if (isLalManagedConnection(settings)) {
    return (
      <Box
        borderStyle="single"
        ...
        width="100%"
      >
        <Text bold>{t('LAL connection needs repair')}</Text>
        <Box marginTop={1} flexDirection="column">
          <Text>{t('Exit LAL, run lal update, then start lal again.')}</Text>
          <Text color={theme.text.secondary}>
            {t('Your project chats and preferences will be preserved.')}
          </Text>
        </Box>
        {authError && (
          <Box marginTop={1}>
