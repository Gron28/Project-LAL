...
import {
  computeRuntimeState,
  runtimeStateLabel,
} from './hooks/use-runtime-state.js';
...
export const AppContainer = (props: AppContainerProps) => {
  ...
  // Typed runtime state replacing the inherited random "witty phrase"
  // cycler: the status line now reflects what the turn loop is actually
  // doing (queued, calling a tool, thinking) instead of a joke picked on a
  // 15s timer. See hooks/use-runtime-state.ts for the state vocabulary and
  // what still needs a wired signal (loading_model, verifying).
  const hasStreamedOutput =
    responseCandidateTokens > 0 || streamingResponseLengthRef.current > 0;
  ...
  useEffect(() => {
    ...
    registry.setNotificationCallback((entry) => {
      ...
      sendNotification(
        { message: `Workflow '${name}' ${verb}`, title: 'LAL' },
        terminal,
        workflowBellEnabled,
      );
    });
