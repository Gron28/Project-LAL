    ...
    () =>
      availableModelEntries.map(
        ({ authType: t2, model, isRuntime, snapshotId }) => {
          ...
          // LAL's own gateway rows are identified by their fixed env key, not
          // by "is this the only USE_OPENAI row" — that heuristic silently
          // stopped holding the moment any other OpenAI-protocol connection
          // (custom provider, another preset) is configured alongside LAL,
          // making the raw "[openai]" tag reappear on LAL's own rows with no
          // way to tell them apart from an unrelated third-party connection.
          const isLalManaged = model.envKey === 'LAL_API_KEY';
          const showProviderTag =
            isRuntime ||
            isQwenOAuth ||
            (!isLalManaged && t2 !== AuthType.USE_OPENAI);
