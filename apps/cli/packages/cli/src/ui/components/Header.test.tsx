...
describe('<Header />', () => {
  ...
  it('hides the ASCII logo on narrow terminal', () => {
    ...
    expect(lastFrame()).toContain('>_ LAL');
  });
  ...
  it('renders the custom subtitle in place of the blank spacer row', () => {
    ...
    const titleIdx = frame.indexOf('>_ LAL');
  ...
  it('renders the custom banner title in place of the default brand', () => {
    ...
    expect(lastFrame()).not.toContain('>_ LAL');
  ...
  it('hides the logo column when neither custom tier fits — does NOT fall back to the default LAL logo (preserves white-label intent)', () => {
    const { lastFrame } = render(
      <Header
        {...defaultProps}
  ...
  it('falls back to the default LAL logo when no custom art was provided at all', () => {
    useTerminalSizeMock.mockReturnValue({ columns: 60, rows: 24 });
    const { lastFrame } = render(<Header {...defaultProps} />);
    // With no customAsciiArt, narrow widths still hide the LAL logo, but a
