...
export class CoreToolScheduler {
      ...
      for (const reqInfo of requestsToProcess) {
        ...
        if (pm && !(await pm.isToolEnabled(canonicalName))) {
          ...
          const ruleInfo = matchingRule
            ...
            : '';
          const permissionErrorMessage = `LAL requires permission to use "${reqInfo.name}", but that permission was declined.${ruleInfo}`;
          newToolCalls.push({
            status: 'error',
            request: reqInfo,
            response: createErrorResponse(
        ...
        if (!pm) {
          ...
          if (excludeTools && excludeTools.length > 0) {
            ...
            if (excludedMatch) {
              const permissionErrorMessage = `LAL requires permission to use ${excludedMatch}, but that permission was declined.`;
              newToolCalls.push({
                status: 'error',
                request: reqInfo,
                response: createErrorResponse(
        ...
        try {
          ...
          if (
            ...
            if (isNonInteractiveDeny) {
              const errorMessage = `LAL requires permission to use "${reqInfo.name}", but that permission was declined (non-interactive mode cannot prompt for confirmation).`;
              this.setStatusInternal(
                reqInfo.callId,
                'error',
                createErrorResponse(
            ...
            if (hooksEnabled && messageBus) {
              fireNotificationHook(
                messageBus,
                `LAL needs your permission to use ${reqInfo.name}`,
  ...
  private bounceToAwaitingApprovalForAsk(
    ...
    if (!this.config.getDisableAllHooks() && messageBus) {
      fireNotificationHook(
        messageBus,
        `LAL needs your permission to use ${toolName}`,
        NotificationType.PermissionPrompt,
        'Permission needed',
      ).catch((error) => {
        debugLogger.warn(
  ...
  private async _executeToolCallBody(
    ...
    const sleepInhibitorHandle = acquireSleepInhibitor(
      this.config,
      `LAL is executing tool ${canonicalName}`,
    );
