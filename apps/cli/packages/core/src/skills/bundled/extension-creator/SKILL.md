...
Use this skill to create LAL extensions with the existing extension
...
LAL hydrates path variables in manifest string fields before
...
LAL discovers command resources recursively from `commands/**/*.md` and
...
After linking, tell the user to restart LAL if the new extension is not
...
- If the extension is missing, inspect the link command output, confirm
  ...
  restart LAL with `QWEN_DEBUG_LOG_FILE` set to a writable log path, then
  inspect that file.
...
5. Restart LAL if the updated extension behavior is not visible in the
   current session.
