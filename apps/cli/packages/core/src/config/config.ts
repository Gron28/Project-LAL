...
import type { CodeModePreset } from '../core/code-mode-presets.js';
...
export interface ConfigParameters {
   ...
   * When set, LAL outputs structured JSON events to this fd while
...
export class Config {
  ...
  // `/mode` bookkeeping: the active named preset (for display/composition with
  // `/effort`) and, when a preset disables thinking, the reasoning value to
  ...
  // Mutable (not readonly): `/mode` rewrites this at runtime so a preset's
  // addendum takes effect via refreshSystemInstruction(), the same live-update
  // pattern setReasoningEffort() uses for the `reasoning` field below.
  ...
  // Mutable (not readonly): `/mode` adjusts the round ceiling per preset at
  // runtime (see setMaxSessionTurns()).
  ...
  /**
   * Run `fn` against every live ContentGeneratorConfig that a request might
   * read from: the main session's config, a sub-agent runtime's config (if
   * distinct), and the rebuildable source consulted on a later refreshAuth/
   * model switch. Mirrors the multi-target update setReasoningEffort() does
   * inline; factored out here so the `/mode` setters below don't duplicate it.
   */
  private forEachLiveContentGeneratorConfig(
    fn: (cfg: ContentGeneratorConfig) => void,
  ): void {
    if (this.contentGeneratorConfig) {
      fn(this.contentGeneratorConfig);
    }
    const runtimeCfg = getRuntimeContentGenerator()?.contentGeneratorConfig;
    if (runtimeCfg && runtimeCfg !== this.contentGeneratorConfig) {
      fn(runtimeCfg);
    }
    const rebuildableSource = this.modelsConfig?.getGenerationConfig();
    if (
      rebuildableSource &&
      rebuildableSource !== this.contentGeneratorConfig
    ) {
      fn(rebuildableSource as ContentGeneratorConfig);
    }
  }

  /**
   * Whether thinking/reasoning is currently enabled (i.e. `reasoning` is not
   * explicitly `false`). Distinct from getReasoningEffort(), which cannot tell
   * "disabled" apart from "no tier chosen, using the model default".
   */
  ...
  /**
   * Enable or disable thinking at runtime (e.g. via `/mode quick-edit`, which
   * disables it). Disabling stashes the current `reasoning` value so a later
   * re-enable restores it (including any `/effort` tier) instead of losing it.
   * Takes effect on the next turn, same as setReasoningEffort().
   */
  setThinkingEnabled(enabled: boolean): void {
    if (!enabled) {
      const current = this.getContentGeneratorConfig()?.reasoning;
      if (current !== false) {
        this.stashedReasoning = current;
      }
    ...
    const restored = this.stashedReasoning;
    this.forEachLiveContentGeneratorConfig((cfg) => {
      cfg.reasoning = restored;
    });
  }

  /**
   * Merge a sampling-params patch (e.g. temperature, max_tokens) into the live
  ...
  /** Override the context-window-size hint used for compaction/truncation math. */
  setContextWindowOverride(size: number | undefined): void {
    this.forEachLiveContentGeneratorConfig((cfg) => {
      cfg.contextWindowSize = size;
    });
  }

  /**
   * Adjust the session turn ceiling at runtime (e.g. `/mode quick-edit` sets a
   * lower ceiling than `/mode orchestrator`). Mirrors the CLI flag's semantics:
   * `<= 0` (well, `< 0` per getMaxSessionTurns' existing checks) means
   * unlimited.
   */
  setMaxSessionTurns(turns: number): void {
    this.maxSessionTurns = turns;
  }

  /**
   * Set the system-prompt addendum at runtime (e.g. a `/mode` preset's
   * instructions). Call `getGeminiClient()?.refreshSystemInstruction()`
   * afterwards to rebind the live chat's system instruction; otherwise this
   * takes effect on the next new session/chat.
   */
  setAppendSystemPrompt(value: string | undefined): void {
    this.appendSystemPrompt = value;
  }

  /** The name of the `/mode` preset currently applied, if any. */
  getActiveCodeMode(): string | undefined {
    return this.activeCodeMode;
  }

  /**
   * Apply a named code-mode preset's budgets/policy to the live session:
   * round ceiling, per-response token ceiling, context-window hint, sampling
   * temperature, thinking on/off, and system-prompt addendum. Mirrors the web
   * app's `MODES` semantics (`web/src/app/api/agent/loop/route.ts`). Does NOT
   * touch the reasoning-effort tier itself — `/effort` composes on top of
   * whatever this sets `think` to (a tier set while thinking is off is stored
   * and reported as not-yet-effective, same as setReasoningEffort() already
   * does; re-enabling thinking picks it back up).
   *
   * Caller is responsible for calling
   * `getGeminiClient()?.refreshSystemInstruction()` afterwards so the
   * addendum takes effect immediately rather than on the next new chat.
   */
