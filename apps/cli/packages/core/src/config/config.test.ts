...
describe('Server Config (config.ts)', () => {
  ...
  describe('applyCodeModePreset (/mode)', () => {
    const preset = {
      label: 'quick-edit',
      maxRounds: 8,
      maxTokens: 2048,
      ...
      think: false,
      temperature: 0,
      addendum: 'MODE: quick-edit — make the smallest correct change.',
    };

    it('applies rounds/tokens/ctx/temperature/thinking/addendum and records the active mode', async () => {
      const config = new Config(baseParams);
      await config.refreshAuth(AuthType.USE_GEMINI);

      config.applyCodeModePreset('quick-edit', preset);

      expect(config.getActiveCodeMode()).toBe('quick-edit');
      expect(config.getMaxSessionTurns()).toBe(8);
      expect(config.getThinkingEnabled()).toBe(false);
      expect(config.getContentGeneratorConfig().samplingParams).toMatchObject({
        temperature: 0,
        max_tokens: 2048,
      });
      expect(config.getContentGeneratorConfig().contextWindowSize).toBe(8192);
      expect(config.getAppendSystemPrompt()).toBe(preset.addendum);
    });

    it('clears the addendum for a preset with an empty one (default)', async () => {
      const config = new Config(baseParams);
      await config.refreshAuth(AuthType.USE_GEMINI);
      ...
      config.applyCodeModePreset('default', { ...preset, addendum: '' });
      expect(config.getAppendSystemPrompt()).toBeUndefined();
    });

    it('composes with /effort: a tier set while a preset disables thinking is stashed, not lost, and resumes once thinking is re-enabled', async () => {
      const config = new Config(baseParams);
      await config.refreshAuth(AuthType.USE_GEMINI);
      ...
      // quick-edit disables thinking — the tier is stashed, not discarded.
      config.applyCodeModePreset('quick-edit', preset);
      expect(config.getThinkingEnabled()).toBe(false);
      expect(config.getReasoningEffort()).toBeUndefined();

      // Switching to a thinking-enabled preset restores it.
      config.applyCodeModePreset('default', {
        ...preset,
        think: true,
        addendum: '',
      });
      expect(config.getThinkingEnabled()).toBe(true);
      expect(config.getReasoningEffort()).toBe('high');
    });

    it('setMaxSessionTurns/setSamplingOverride/setContextWindowOverride/setAppendSystemPrompt round-trip independently of a preset', async () => {
      const config = new Config(baseParams);
      await config.refreshAuth(AuthType.USE_GEMINI);

      config.setMaxSessionTurns(42);
      expect(config.getMaxSessionTurns()).toBe(42);

      config.setSamplingOverride({ temperature: 0.7 });
      expect(
        config.getContentGeneratorConfig().samplingParams?.temperature,
      ).toBe(0.7);

      config.setContextWindowOverride(32768);
      expect(config.getContentGeneratorConfig().contextWindowSize).toBe(32768);
