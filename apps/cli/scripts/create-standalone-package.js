...
const ROOT_REQUIRED_PATHS = ['LICENSE', 'NOTICE-LAL.md'];
...
async function main() {
  ...
  const outputName = `lal-cli-${target}.${targetConfig.outputExtension}`;
  const outputPath = path.join(outDir, outputName);
  const tempRoot = fs.mkdtempSync(path.join(os.tmpdir(), 'lal-standalone-'));

  try {
    const packageRoot = path.join(tempRoot, 'lal-cli');
...
function printUsage() {
  console.log(`LAL standalone package builder
...
Options:
  ...
  --version VERSION       LAL version. Defaults to package.json version.
...
function copyRuntimeAssets(packageRoot, outDir) {
  ...
  fs.copyFileSync(
    path.join(rootDir, 'README-LAL.md'),
  ...
  fs.writeFileSync(
    ...
    JSON.stringify(
      {
        name: '@local-ai-lab/lal-cli-standalone',
...
`;
  const unixShimPath = path.join(binDir, 'lal');
...
exit /b %ERRORLEVEL%
`;
  fs.writeFileSync(path.join(binDir, 'lal.cmd'), windowsShim);
}
...
function createArchive(outputExtension, outputPath, cwd) {
  ...
  run('tar', ['-czf', outputPath, '-C', cwd, 'lal-cli']);
}
...
function createZipArchive(outputPath, cwd) {
  if (process.platform === 'win32') {
    run(
      ...
      ],
      {
        env: {
          ...process.env,
          QWEN_PACKAGE_ROOT: path.join(cwd, 'lal-cli'),
  ...
  run('zip', ['-qr', outputPath, 'lal-cli'], { cwd });
}
...
async function writeSha256Sums(outDir) {
  ...
  if (entries.length === 0) {
    fail(
      `No LAL archives found in ${outDir}; refusing to write empty SHA256SUMS.`,
    );
  }
