...
async function main() {
  ...
  const releaseTargets = args.target
    ? RELEASE_TARGETS.filter(({ qwenTarget }) => qwenTarget === args.target)
    : RELEASE_TARGETS;
  if (releaseTargets.length === 0) {
    fail(
      `Unknown target ${args.target}. Expected one of: ${RELEASE_TARGETS.map(({ qwenTarget }) => qwenTarget).join(', ')}`,
    );
  }
  ...
  try {
    ...
    const nativeModulesDir = stageClipboardPackages(runtimeDir, releaseTargets);
    ...
    assertStandaloneOutput(outDir, releaseTargets);
  } finally {
...
function readClipboardPackageSpecs(releaseTargets = RELEASE_TARGETS) {
  ...
  const packageNames = [
    '@teddyzhu/clipboard',
    ...new Set(
      releaseTargets.map(({ qwenTarget }) =>
        TARGET_CLIPBOARD_PACKAGE.get(qwenTarget),
      ),
    ),
  ];
...
function stageClipboardPackages(runtimeDir, releaseTargets) {
  ...
  execFileSync(
    ...
    [
      ...
      '--no-audit',
      '--no-fund',
      ...readClipboardPackageSpecs(releaseTargets),
    ],
    {
      cwd: rootDir,
      stdio: 'inherit',
...
function assertStandaloneOutput(outDir, releaseTargets = RELEASE_TARGETS) {
  ...
  const archiveNames = fs
    ...
    .filter(Boolean)
    .sort();
  const expectedArchiveNames = releaseTargets
    .map(
      ({ qwenTarget }) =>
        `lal-cli-${qwenTarget}.${qwenTarget === 'win-x64' ? 'zip' : 'tar.gz'}`,
    )
    .sort();
...
Options:
  ...
  --target TARGET        Package one target only (for example win-x64).
`);
}
