...
type DownloadGlobal = typeof globalThis & { __lal_model_downloads?: Map<string, Promise<ModelDownloadExecution>> };
...
export function requestModelDownload(request: ModelDownloadRequest): Job {
  ...
  const job = repository.create({ id: jobId, kind: "model.download", requestedBy: request.requestedBy || "local-browser", capabilityScope: ["model.download"], resources: { network: true, diskBytes: request.plan.requiredDiskBytes, cpuSlots: 1 }, restartPolicy: "resume", checkpoint: { plan: request.plan, modelName: request.modelName, importId: jobId }, inputs: [], retentionClass: "model-acquisition" });
