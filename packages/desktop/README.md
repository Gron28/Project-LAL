...
LAL is a desktop and headless agent workspace. It provides multi-session chat, source connections, skills, file previews, automations, and permission modes in a local-first application.

## Backend

This fork is Qwen-only:

- Agent sessions run through LAL over ACP.
...
The desktop app talks to the LAL CLI over ACP. Treat the CLI as a
...
the LAL repository and point desktop at it:
...
1. **`electron:vendor:qwen`** — vendors a LAL CLI runtime into `vendor/qwen-code/`. Set `QWEN_CODE_VERSION` to download a published npm version, or `QWEN_CODE_TARBALL` to use a local `npm pack` tarball. If neither is set in this monorepo, it builds from the local checkout.
...
The `run` command spawns a headless server, creates a temporary session, streams the response, and exits. Provider flags are accepted only for compatibility; the backend remains LAL.
...
- LAL model discovery through ACP
