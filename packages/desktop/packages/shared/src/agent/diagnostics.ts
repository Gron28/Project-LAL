 ...
 *
 * LAL is the only built-in backend, so diagnostics focus on captured
...
export async function runErrorDiagnostics(config: DiagnosticConfig): Promise<DiagnosticResult> {
  const providerLabel = config.providerType === 'qwen' ? 'LAL' : 'Backend';
  ...
  return {
    ...
    message: `${providerLabel} is unavailable. Check the LAL CLI installation and try again.`,
    details,
  };
}
