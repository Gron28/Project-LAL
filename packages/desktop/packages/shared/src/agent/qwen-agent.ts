...
/**
 * LAL Backend (ACP SDK Client)
 *
 * Spawns LAL in ACP mode and adapts ACP session updates into Craft's
...
async function callQwenSettingsAcpMethod(
  ...
  if (!qwenCliPath) {
    throw new Error(
      'LAL CLI not found. Build the current qwen-code checkout with npm run build && npm run bundle, or set QWEN_CODE_CLI to a dist/cli.js path.',
    );
  }
...
export class QwenAgent extends BaseAgent {
  ...
  private handlePermissionRequest(
    ...
    const title = asString(toolCall.title) || 'LAL requests permission';
    const toolName = normalizeToolName(
