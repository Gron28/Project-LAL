  ...
  const saveQwenConnection = useCallback(async (connectionSlugOverride?: string): Promise<boolean> => {
    ...
    if (!result.success) {
      setState(s => ({
        ...
        completionStatus: 'saving',
        errorMessage: result.error || 'Failed to save LAL configuration',
      }))
      return false
    }
    ...
    try {
      ...
      if (!testResult.success) {
        setState(s => ({
          ...s,
          credentialStatus: 'error',
          errorMessage: testResult.error || 'LAL connection test failed',
        }))
        return
      }
      ...
      setState(s => ({
        ...s,
        credentialStatus: 'error',
        errorMessage: error instanceof Error ? error.message : 'LAL validation failed',
      }))
    }
  }, [saveQwenConnection])
