#!/usr/bin/env bun
/**
 * craft-cli — Terminal client for LAL server.
 *
 * Connects over WebSocket (ws:// or wss://) to a running LAL server
...
function getProviderDisplayName(provider: string): string {
  return provider === 'qwen' ? 'LAL' : provider.charAt(0).toUpperCase() + provider.slice(1)
}
...
async function setupLlmConnection(
  ...
  process.stderr.write('LLM connection configured: LAL\n')
...
async function cmdRun(args: CliArgs): Promise<void> {
  ...
  try {
    ...
    // Auto-setup the LAL connection when no connection exists yet.
  ...
  return [
    ...
    },
    {
      ...
      fn: async (client, ctx) => {
        ...
        return '0 found -> created LAL connection'
      },
    },
    {
      name: 'sources:get',
...
function printHelp(): void {
  process.stdout.write(`craft-cli — Terminal client for LAL server
...
LLM Configuration (for 'run' command):
  ...
  --api-key <key>        Ignored for LAL; kept for script compatibility
  --base-url <url>       Ignored for LAL; kept for script compatibility
