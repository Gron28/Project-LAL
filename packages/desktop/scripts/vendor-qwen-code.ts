...
function verifyVendoredCli(): void {
  ...
  if (!hasRootCli && !hasDistCli) {
    throw new Error(
      `LAL CLI not found in ${vendorDir}. Expected cli.js or dist/cli.js.`,
    );
  }
}
...
async function vendorLocalCheckout(repoRoot: string): Promise<void> {
  if (!isQwenSourceRoot(repoRoot)) {
    throw new Error(
      `LAL source checkout not found at ${repoRoot}. Set QWEN_CODE_VERSION, QWEN_CODE_TARBALL, or QWEN_CODE_ROOT.`,
    );
  }

  console.log(`Building LAL CLI from ${repoRoot}...`);
  ...
  if (!existsSync(join(localDistDir, 'cli.js'))) {
    throw new Error(
      `Local LAL bundle not found at ${join(localDistDir, 'cli.js')}.`,
    );
  }
  ...
  verifyVendoredCli();
  console.log(`Vendored local LAL CLI into ${vendorDir}`);
}
...
async function resolveNpmVersionOrTag(
  ...
  if (!requested) {
    throw new Error('LAL npm version or dist-tag is required.');
  }
...
async function vendorNpmVersion(versionOrTag: string): Promise<void> {
  ...
  console.log(`Downloading LAL ${sourceLabel} from npm...`);
...
async function vendorTarball(tarballPath: string): Promise<void> {
  ...
  if (!existsSync(source)) {
    throw new Error(`LAL tarball not found: ${source}`);
  }

  console.log(`Vendoring LAL from tarball ${source}...`);
  ...
  verifyVendoredCli();
  console.log(`Vendored LAL tarball into ${vendorDir}`);
}
