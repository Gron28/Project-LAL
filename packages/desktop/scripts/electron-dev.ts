...
async function ensureQwenRuntimeForDev(): Promise<void> {
  if (
    ...
    console.log('🧭 Using LAL runtime override from environment');
    return;
  }
  ...
  if (!requestedExternalArtifact && isQwenSourceRoot(monorepoRoot)) {
    console.log('🧭 Using LAL CLI from the local monorepo checkout');
    return;
  }
  ...
  if (vendoredCli && !requestedExternalArtifact) {
    ...
    console.log(`🧭 Using vendored LAL CLI: ${vendoredCli}`);
    return;
  }
  ...
  if (!env.QWEN_CODE_TARBALL && !env.QWEN_CODE_VERSION) {
    ...
    if (!defaultVersion) {
      throw new Error(
        'No LAL CLI runtime configured. Set QWEN_CODE_VERSION, QWEN_CODE_TARBALL, QWEN_CODE_ROOT, or qwenCodeRuntime.version in package.json.',
      );
    }
  ...
  console.log(`📦 Vendoring LAL CLI from ${sourceLabel}...`);
  ...
  if (exitCode !== 0) {
    throw new Error(`Failed to vendor LAL CLI from ${sourceLabel}`);
  }

  const resolvedCli = firstExistingPath(QWEN_VENDOR_CLI_CANDIDATES);
  if (!resolvedCli) {
    throw new Error(`Vendored LAL CLI not found in ${QWEN_VENDOR_DIR}`);
  }
  process.env.QWEN_CODE_CLI = resolvedCli;
  console.log(`🧭 Using vendored LAL CLI: ${resolvedCli}`);
}
...
function getElectronEnv(): Record<string, string> {
  ...
  return {
    ...
    CRAFT_APP_NAME: process.env.CRAFT_APP_NAME || 'LAL Desktop',
