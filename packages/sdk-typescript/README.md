...
A minimum experimental TypeScript SDK for programmatic access to LAL.
...
Creates a new query session with the LAL.
...
| `env`                    | `Record<string, string>`                       | -                | Environment variables to pass to the LAL process. Merged with the current process environment.                                                                                                                                                                                                                                                                                                                                                                                  |
| `systemPrompt`           | `string \| QuerySystemPromptPreset`            | -                | System prompt configuration for the main session. Use a string to fully override the built-in LAL system prompt, or a preset object to keep the built-in prompt and append extra instructions.                                                                                                                                                                                                                                                                                  |
...
#### LAL Installation Required

Version 0.1.0 requires [LAL](https://github.com/QwenLM/qwen-code) **>= 0.4.0** to be installed separately and accessible in your PATH.

```bash
# Install LAL globally
npm install -g @qwen-code/qwen-code@latest
```

**Note**: From version **0.1.1** onwards, the CLI is bundled with the SDK, so no separate LAL installation is needed.
