...
The LAL Java SDK is a minimum experimental SDK for programmatic access to LAL functionality. It provides a Java interface to interact with the LAL CLI, allowing developers to integrate LAL capabilities into their Java applications.
...
- **Session Layer**: Manages communication sessions with the LAL CLI through the `Session` class
...
The project includes basic unit tests using JUnit 5. The main test class `QwenCodeCliTest` demonstrates how to use the SDK to make simple queries to the LAL CLI.
...
The `TransportOptions` class allows configuration of how the SDK communicates with the LAL CLI:

- `pathToQwenExecutable`: Path to the LAL CLI executable
