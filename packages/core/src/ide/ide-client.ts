...
export class IdeClient {
  ...
  async connect(): Promise<void> {
    ...
    if (this.workspaceRejectedPorts.size > 0) {
      this.setState(
        IDEConnectionStatus.Disconnected,
        `Found IDE companion extension, but its workspace does not match the current directory. Run LAL from the workspace open in your IDE, or switch the IDE to this project.`,
        true,
      );
      return;
    }
  ...
  static validateWorkspacePath(
    ...
    if (!isWithinWorkspace) {
      return {
        isValid: false,
        error: `Directory mismatch. LAL is running in a different location than the open workspace in the IDE. Please run the CLI from one of the following directories: ${ideWorkspacePaths.join(
          ', ',
        )}`,
      };
    }
