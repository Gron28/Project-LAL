...
> {
  ...
  async execute(
    ...
    // Add co-author to git commit commands and LAL attribution to
   ...
   * PR body. Format: "Generated with LAL (N-shotted by Qwen-Coder)"
   * when at least one user prompt has been recorded since the last commit;
   * otherwise just "Generated with LAL".
   *
  ...
  private addAttributionToPR(command: string): string {
    ...
    const attribution =
      shots > 0
        ? `\n\nGenerated with LAL (${shots}-shotted by ${generator})`
        : `\n\nGenerated with LAL`;
