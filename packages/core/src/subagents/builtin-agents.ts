...
- For clear communication with the user the assistant MUST avoid using emojis.`,
    ...
    },
    {
      ...
      description:
        "Use this agent to configure the user's LAL status line setting.",
      tools: [
        ToolNames.READ_FILE,
      ...
      color: 'orange',
      systemPrompt: `You are a status line setup agent for LAL. Your job is to create or update the statusLine command in the user's LAL settings.
...
WILL corrupt settings.json and prevent LAL from starting.
...
- IMPORTANT: At the end of your response, remind the user that they can ask LAL to make further changes to the status line at any time.
`,
    },
  ];
