...
export class PanelManager {
  ...
  async createPanel(): Promise<boolean> {
    ...
    // First, check if there's an existing LAL group
    const existingGroup = this.findExistingQwenCodeGroup();

    if (existingGroup) {
      // If LAL webview already exists in a locked group, create the new panel in that same group
      console.log(
        '[PanelManager] Found existing LAL group, creating panel in same group',
      );
      ...
      // If no existing LAL group, create a new group to the right of the active editor group
      try {
  ...
  /**
   * Find the group and view column where the existing LAL webview is located
    ...
    for (const group of vscode.window.tabGroups.all) {
      for (const tab of group.tabs) {
        ...
        if (
          ...
          // Found an existing LAL tab
          console.log('[PanelManager] Found existing LAL group:', {
