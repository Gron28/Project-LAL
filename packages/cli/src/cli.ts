...
export const TOP_LEVEL_COMMANDS = [
  ...
  ['extensions <command>', 'Manage LAL extensions.'],
  ['hooks', 'Manage LAL hooks (use /hooks in interactive mode).'],
  ...
  [
    'serve',
    'Run LAL as a local HTTP daemon (Stage 1 experimental: --http-bridge)',
  ],
  ['sessions <command>', 'Manage LAL sessions'],
  ['update', 'Check for LAL updates and install if available'],
] as const;
...
async function buildTopLevelHelpParser() {
  ...
  const parser = yargs([])
    .scriptName('lal')
    .usage(
      'Usage: lal [options] [command]\n\nLAL - Launch an interactive CLI, use -p/--prompt for non-interactive mode',
    )
...
function printMcpHelp(): void {
  const lines = [
    'Usage: lal mcp <command>',
    '',
    'Manage MCP servers',
    '',
    'Commands:',
    ...MCP_COMMANDS.map(
      ([command, description]) => `  lal mcp ${command}  ${description}`,
    ),
  ];
