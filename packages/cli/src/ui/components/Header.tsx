...
interface HeaderProps {
   ...
   * then falls through to the default LAL logo. Either tier may be
   * omitted: a missing tier simply skips that step.
   */
  customAsciiArt?: { small?: string; large?: string };
  /**
   * Sanitized replacement for the bold ">_ LAL" title in the info
...
export const Header: React.FC<HeaderProps> = ({
  ...
  //     Falling back to the bundled LAL logo here would silently undo a
  ...
  const gradientColors = getRenderableGradientColors(LAL_BRAND_GRADIENT, [
    ...
    >
      ...
      >
        {/* Title line: customBannerTitle (already sanitized) or the default
            ">_ LAL" brand. Version suffix is always appended. */}
        <Text>
          <Text
            bold
            color={customBannerTitle ? theme.text.accent : LAL_BRAND_GREEN}
          >
            {customBannerTitle ? customBannerTitle : '>_ LAL'}
          </Text>
          <Text color={LAL_BRAND_YELLOW}> ({versionLabel})</Text>
        </Text>
