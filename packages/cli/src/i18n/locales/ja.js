...
// Japanese translations for LAL CLI
...
export default {
  ...
  'List available LAL tools. Usage: /tools [desc]':
    '利用可能な LAL ツールを一覧表示。使い方: /tools [desc]',
  ...
  Bundled: '組み込み',
  'Available LAL CLI tools:': '利用可能な LAL CLI ツール:',
  ...
  'Generate with LAL (Recommended)': 'LAL で生成(推奨)',
  ...
  'To see changes, LAL must be restarted. Press r to exit and apply changes now.':
    '変更を確認するには LAL を再起動する必要があります。 r を押して終了し、変更を適用してください',
  'View and edit LAL settings': 'LAL の設定を表示・編集',
  Settings: '設定',
  'Vim Mode': 'Vim モード',
  'Output Format': '出力形式',
  'Hide Tips': 'ヒントを非表示',
  ...
  'You are installing an extension from {{originSource}}. Some features may not work perfectly with LAL.':
    '{{originSource}} から拡張機能をインストールしています。一部の機能は LAL で完全に動作しない可能性があります。',
  ...
  'IDE integration is not supported in your current environment. To use this feature, run LAL in one of these supported IDEs: VS Code or VS Code forks.':
    '現在の環境ではIDE連携はサポートされていません。この機能を使用するには、VS Code または VS Code 派生エディタで LAL を実行してください',
  ...
  // Commands - Hooks
  // ============================================================================
  'Manage LAL hooks': 'LAL のフックを管理する',
  ...
  'This menu is read-only. To add or modify hooks, edit settings.json directly or ask LAL.':
    'このメニューは読み取り専用です。フックを追加または変更するには、settings.json を直接編集するか、LAL に尋ねてください。',
  ...
  'To re-enable hooks, remove "disableAllHooks" from settings.json or ask LAL.':
    'フックを再有効化するには、settings.json から "disableAllHooks" を削除するか、LAL に尋ねてください。',
  // Hooks - Source
  Project: 'プロジェクト',
  User: 'ユーザー',
  Skill: 'スキル',
  ...
  'You can run any shell commands from LAL using ! (e.g. !ls).':
    'LAL から ! を使って任意のシェルコマンドを実行できます（例: !ls）。',
  ...
  'Add a QWEN.md file to give LAL persistent project context.':
    'QWEN.md ファイルを追加すると、LAL に永続的なプロジェクトコンテキストを与えられます。',
  ...
  "Set up LAL's status line UI": 'LAL のステータスライン UI を設定',
  ...
  'Check for LAL updates and install if available':
    'LALのアップデートを確認し、利用可能な場合はインストールします',
  'LAL update available! {{current}} → {{latest}}':
    'LAL のアップデートがあります！{{current}} → {{latest}}',
  'A new version of LAL is available! {{current}} → {{latest}}':
    'LAL の新しいバージョンがあります！{{current}} → {{latest}}',
  'LAL {{version}} is up to date!': 'LAL {{version}} は最新です！',
  ...
  'Update successful! Please restart LAL to use the new version. Switching model providers before restarting may not work correctly.':
    'アップデートに成功しました！新しいバージョンを使用するには LAL を再起動してください。再起動前にモデルプロバイダーを切り替えると正しく動作しない場合があります。',
