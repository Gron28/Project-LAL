...
// Chinese translations for LAL CLI
...
export default {
  ...
  'Clipboard image paste is unavailable because the native clipboard module could not be loaded. Reinstall LAL or use the npm installation method.':
    '剪贴板图片粘贴不可用，因为原生剪贴板模块加载失败。请重新安装 LAL，或改用 npm 安装方式。',
  ...
  'Generate with LAL (Recommended)': '使用 LAL 生成（推荐）',
  ...
  'You are installing an extension from {{originSource}}. Some features may not work perfectly with LAL.':
    '您正在安装来自 {{originSource}} 的扩展。某些功能可能无法完美兼容 LAL。',
  ...
  'IDE integration is not supported in your current environment. To use this feature, run LAL in one of these supported IDEs: VS Code or VS Code forks.':
    '您当前环境不支持 IDE 集成。要使用此功能，请在以下支持的 IDE 之一中运行 LAL：VS Code 或 VS Code 分支版本。',
  ...
  'This menu is read-only. To add or modify hooks, edit settings.json directly or ask LAL.':
    '此菜单为只读。要添加或修改 Hook，请直接编辑 settings.json 或询问 LAL。',
  ...
  'To re-enable hooks, remove "disableAllHooks" from settings.json or ask LAL.':
    '要重新启用 Hook，请从 settings.json 中删除 "disableAllHooks" 或询问 LAL。',
  // Hooks - Source
  Project: '项目',
  User: '用户',
  Skill: '技能',
  ...
  'Right before LAL concludes its response': 'LAL 结束响应之前',
  ...
  'Voice dictation needs microphone access, which is unavailable in this WSL session. Use WSLg/PulseAudio, or run LAL on a host with a microphone.':
    '语音听写需要麦克风访问，但当前 WSL 会话不可用。请使用 WSLg/PulseAudio，或在带麦克风的宿主机上运行 LAL。',
  ...
  'You can run any shell commands from LAL using ! (e.g. !ls).':
    '在 LAL 中使用 ! 可运行任意 shell 命令（例如 !ls）。',
  ...
  'Add a QWEN.md file to give LAL persistent project context.':
    '添加 QWEN.md 文件，为 LAL 提供持久的项目上下文。',
  ...
  // Exit Screen / Stats
  // ============================================================================
  'Agent powering down. Goodbye!': 'LAL 正在关闭，再见！',
  ...
  "Set up LAL's status line UI": '配置 LAL 的状态栏',
  ...
  'Update successful! Please restart LAL to use the new version. Switching model providers before restarting may not work correctly.':
    '更新成功！请重启 LAL 以使用新版本。重启前切换模型提供商可能无法正常工作。',
