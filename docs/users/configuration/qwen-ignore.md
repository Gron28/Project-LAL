...
This document provides an overview of the Qwen Ignore (`.qwenignore`) feature of LAL. LAL also recognizes custom ignore files configured by `context.fileFiltering.customIgnoreFiles`, which defaults to the compatibility files `.agentignore` and `.aiignore`.

LAL includes the ability to automatically ignore files, similar to `.gitignore` (used by Git). Adding paths to `.qwenignore` or a configured custom ignore file will exclude them from tools that support this feature, although they will still be visible to other services (such as Git).
...
You can update these ignore files at any time. To apply the changes, you must restart your LAL session.
...
By default, LAL reads `.qwenignore`, `.agentignore`, and `.aiignore`.
