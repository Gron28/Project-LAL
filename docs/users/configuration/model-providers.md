...
LAL allows you to configure multiple model providers through the `modelProviders` setting in your `settings.json`. This enables you to switch between different AI models and providers using the `/model` command.
...
LAL uses the following official SDKs to send requests to each provider:
...
When you authenticate with an Alibaba Cloud Coding Plan API key using the `/auth` command, LAL automatically configures the following models:
...
Coding Plan model configurations are versioned. When LAL detects a newer version of the model template, you will be prompted to update. Accepting the update will:
...
LAL distinguishes between two types of model configurations:
...
When you configure a model without using `modelProviders`, LAL automatically creates a RuntimeModelSnapshot to preserve your configuration:
