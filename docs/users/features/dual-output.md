...
Dual Output is a sidecar mode for the interactive TUI: while LAL keeps
...
Embed LAL inside the IDE. The TUI runs in the editor's integrated
...
A CI job runs LAL with a task prompt. The human sees the TUI in the
...
Integration tests spawn LAL headlessly, drive it with `--input-file`
...
At first glance `--json-fd` looks sufficient — the caller spawns LAL
...
Run LAL with both channels enabled using regular files:
...
A typical embedding parent process spawns LAL with both channels:
...
next LAL launch, since the bridge is constructed once during
startup.
...
The most realistic shape: a parent process spawns LAL, tails
...
# LAL TUI opens in the current terminal; the embedder logs
# handshake + turn-end + session_end events to the parent's stdout.
```

### POC 5 — capability handshake feature detection

Older LAL versions won't emit `protocol_version`. Treat the field
