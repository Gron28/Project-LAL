...
Channels let you interact with a LAL agent from messaging platforms like Telegram, WeChat, QQ, DingTalk, WeCom, or Feishu, instead of the terminal. You send messages from your phone or desktop chat app, and the agent responds just like it would in the CLI.

## How It Works

When you run `qwen channel start`, LAL:
...
Channel memory lets accepted channel senders save stable context for one chat or thread. LAL injects that memory when a fresh channel session starts, including after `/clear`.
