...
LAL is built for multilingual workflows: it supports UI localization (i18n/l10n) in the CLI, lets you choose the assistant output language, and allows custom UI language packs.

## Overview

From a user point of view, LAL’s “internationalization” spans multiple layers:
...
On first startup, LAL detects your system locale and sets the UI language automatically.
...
On first startup, if no `output-language.md` file exists, LAL automatically creates one based on your system locale. For example:
...
> [!note]
>
> After changing the output language, restart LAL for the change to take effect.
