...
Headless mode allows you to run LAL programmatically from command line
scripts and automation tools without any interactive UI. This is ideal for
scripting, automation, CI/CD pipelines, and building AI-powered tools.

## Overview

The headless mode provides a headless interface to LAL that:
...
Pipe input to LAL from your terminal:
...
Read from files and process with LAL:
...
Use `--system-prompt` to replace LAL's built-in main-session prompt for the current run:
...
LAL supports multiple output formats for different use cases:
...
The `--input-format` parameter controls how LAL consumes input from standard input:
...
When LAL runs in CI/CD pipelines or as a background daemon, a brief API outage (rate limiting or overload) should not kill a multi-hour task. **Persistent retry mode** makes LAL retry transient API errors indefinitely until the service recovers.
