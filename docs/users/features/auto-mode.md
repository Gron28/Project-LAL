...
When you're in Auto Mode and the agent tries to run a tool, LAL
...
1. **acceptEdits fast-path** — Edit / Write whose target path is inside
   ...
   **Exception:** writes to LAL's own self-modification surfaces
...
- `permissions.allow` rules with specific specifiers (e.g.
  ...
  into permission to rewrite LAL settings, commands, hooks,
...
When you enter Auto Mode, LAL temporarily removes these rules from
  ...
  "permissions": {
    "autoMode": {
      "hints": {
        ...
        "softDeny": [
          "Editing LAL settings unless I explicitly ask for the exact change",
...
the main agent. If you've configured LAL to use a self-hosted
