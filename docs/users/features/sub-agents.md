...
Subagents are specialized AI assistants that handle specific types of tasks within LAL. They allow you to delegate focused work to AI agents that are configured with task-specific prompts, tools, and behaviors.
...
- `glm-5`: Use that model ID. LAL first checks the main conversation's
...
When the selector resolves to another auth type, LAL creates a dedicated
...
`agents.builtin.exploreModel` in `settings.json` and restart LAL:
...
when LAL resolves the built-in Explore definition; a session, project,
...
LAL accepts the Claude Code 2.1.168 frontmatter fields below so you
...
LAL proactively delegates tasks based on:
