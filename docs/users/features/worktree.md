...
Alternatively, ask LAL in plain language to create a worktree from inside an existing session:
...
The `agent` tool accepts an optional `isolation: "worktree"` parameter. When set, LAL creates an ephemeral worktree at `<repoRoot>/.qwen/worktrees/agent-<7hex>/` before the sub-agent starts, and:
