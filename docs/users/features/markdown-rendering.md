...
LAL renders common Markdown structures directly in the TUI so model
...
To start LAL in raw mode by default, set `ui.renderMode`:
...
1. If enabled and supported, LAL asks Mermaid CLI (`mmdc`) to render the
...
3. Otherwise, LAL falls back to a terminal wireframe or compact text
   preview.
4. If a Mermaid diagram type cannot be previewed, LAL shows the original
...
| `QWEN_CODE_MERMAID_ALLOW_NPX=1`             | Allows LAL to run `npx @mermaid-js/mermaid-cli` when `mmdc` is not installed.     |
...
download Mermaid CLI. During streaming, LAL shows a bounded text preview
...
LAL supports basic inline and block LaTeX rendering in the terminal:
