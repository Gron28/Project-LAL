...
The prototype channel does not yet publish a remote uninstall script. Remove only
the managed launcher/runtime. Preserve `~/.lal` if you want to keep chats and
device identity.

**Linux / macOS**

```bash
rm -f "$HOME/.local/bin/lal"
```

**Windows**

```powershell
Remove-Item -Recurse -Force "$env:LOCALAPPDATA\LAL"
```

Your LAL configuration and project chats live in `~/.lal` and are not removed by
those commands.
